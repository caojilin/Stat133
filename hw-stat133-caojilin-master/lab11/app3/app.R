...
ui <- fluidPage(
  ...
  # Sidebar with a slider input for number of repeats 
  sidebarLayout(
                sidebarPanel(
                  sliderInput("repeats",
                ...
                sliderInput("integer", "threshold for choosing the boxes:",
                            min = 0, max = 1,
                            value = 0.5),
  
                selectInput(inputId = "seed",
                            label = "Choose a random seed:",
                            choices = c("1235", "54321", "3345"))
                ),
...
server <- function(input, output) {
  draw = function(repetition){
    ...
    seeds = as.numeric(input$seed)
    set.seed(seeds)
    for(i in 1:repetition){
      #select a box
      num = runif(1)
      if(num > input$integer){
